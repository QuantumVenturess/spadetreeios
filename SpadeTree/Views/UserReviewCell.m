...
- (void) loadReviewData: (Review *) reviewObject
{
  review = reviewObject;
  // Image view
  if (review.tutee.image) {
    imageView.image = [review.tutee imageWithSize: CGSizeMake(40, 40)];
  }
  else {
    [review.tutee downloadImage: ^(NSError *error) {
      if (!error) {
        imageView.image = [review.tutee imageWithSize: CGSizeMake(40, 40)];
      }
    }];
  ...
  // Name
  nameLabel.text = [review.tutee fullName];
  // Created
  createdLabel.text = [review createdDateStringShort];
  // Content
